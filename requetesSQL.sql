...
select t.num_trophee , t.date_prise , t.num_preneur  from trophee t ;
...
and a.date_a between '2052-02-01' and '2052-02-28';		-- dans un intervale de temps.
...
-- Or, la solution trouvée ne répondait pas à la question : on recherche une date au formalisme française, ... et non américaine
...
select distinct v.nom_village, count(h.nom)
from habitant h	-- Il nous faut une Sélection de "v.nom_village" et "h.nom" - Seul le tableau 'absorber' est paramétré.
join village v on v.num_village = h.num_village	-- ceci est une jointure entre les tables 'village' et 'habitant'
group by v.nom_village, h.nom ;  -- pour l'affichage dès que la sélection est multiple.
...
select distinct  h.nom, count(t.num_trophee) 
